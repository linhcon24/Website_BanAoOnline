...
<h1 style="text-align: center;"><i class="fa-solid fa-file-invoice"></i> Quản Lý Danh Sách Yêu Thích</h1>
<br>
<br>
<table class="table">
    <tr>
    ...
    <c:forEach items="${listDSYT}" var="p">
        <%--            <c:set value="${i+1 }" var="i" scope="page" ></c:set>--%>
        <c:if test="${p.trangthai == 0 }">
            <tr>
                    <%--                    <td>${i }</td>--%>
                <td>${p.iddanhsachyt}</td>
                <td>${p.madanhsachyt}</td>
                <td>${p.tendanhsachyt}</td>
                <td>${p.ngaytao}</td>
                <td>${p.ngaycapnhat}</td>
                <td>${p.trangthai}</td>
